...
而不是 2MB 的页大小的原因，这个问题即使不用 JavaScript 用机器码也存在。
...
图6 显示的内存谱图捕捉到了这一隐藏信道的执行。隐藏信道的理论带宽通过测量大约是 320kbps，这和 文章[23] 中用机器码实现的跨虚拟机的隐藏信道
1.2Mbps 的带宽比较吻合。
...
在**微结构**这层，修改物理内存到缓存线的映射方式可以非常有效地阻止我们的攻击，即不再用地址底12比特中的6个直接选择一个组。类似的，换用非嵌套的
缓存微结构而不是用嵌套的,会让我们的代码几乎不肯能精确地一级缓存中淘汰某项，使得测量更加困难。然而,这两个设计决定当初被选择正是为了让 CPU 的设计
